...
### لماذا `db push` لا `migrate deploy`؟

الهجرات (migrations) في Prisma عبارة عن ملفات **SQL خاصة بمزوّد واحد**:
هجرة مولّدة لـ SQLite لا تعمل على MySQL والعكس. وبما أن هذا المشروع مصمّم
ليعمل على الأنواع الثلاثة (`npm run db:sqlite | db:mysql | db:postgres`)،
فإن الاحتفاظ بمجلد هجرات واحد يكسر هذه الميزة.

`db push` يقرأ المخطط ويُنشئ الجداول مباشرةً بأي مزوّد — وهو الصحيح ما دمت
مالك القاعدة الوحيد.

**متى تنتقل إلى الهجرات؟** عندما تستقرّ على قاعدة واحدة نهائياً وتحتاج
سجلّاً للتغييرات (فريق عمل، أو عدّة بيئات). حينها:

```bash
npx prisma migrate dev --name init      # مرة واحدة، بعد تثبيت المزوّد
```

وبعدها استعمل `npm run db:deploy` بدل `db push`.
...
```bash
npx prisma db push   # يطبّق أي تغييرات جديدة في المخطط
```

وأعد تشغيل التطبيق.

> ⚠️ **قبل أي `db push` على قاعدة فيها بيانات حقيقية**: خذ نسخة احتياطية من
> cPanel أو من `/settings/backup`. الأمر يعدّل الجداول مباشرةً، وإن اكتشف
> اختلافاً يستدعي حذف عمود فسيطلب `--accept-data-loss`. لا تمرّر هذا الخيار
> دون أن تفهم ما الذي سيُحذف.
